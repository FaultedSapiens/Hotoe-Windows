...
HWND EnsureInputPiece(int pieceIndex)
{
    ...
    while (static_cast<int>(gInputPieces.size()) <= pieceIndex)
    {
        ...
        if (!piece.hwnd)
        {
            Log("P6F1 Create dynamic input HWND FAILED");
            return nullptr;
        }
...
)
{
    ...
    /*
        P6F1 no longer allocates exactly four HWNDs.
    ...
    */
    ...
    Log("P6F1 native topology: dynamic input-surface pool ready");
    return true;
}
...
) -> HRESULT
{
    ...
    {
        Log(
            "D3 WebMessageReceived returned null JSON"
        );
    ...
    // Preserve JSON before releasing WebView2-owned memory.
    CoTaskMemFree(
        rawJson
    );

    // ========================================================
    // P6F1 authoritative browser snapshot
    //
    // Current HTML sends ONE message:
    //
    // {
    //   "type":"regions",
    //   "generation":g,
    //   "regions":[
    //      {"id":"s0","rect":{"left":...,"top":...,"right":...,"bottom":...}},
    //      ...
    //   ]
    // }
    //
    // F1 intentionally treats every reported support as a
    // rectangle. In particular S2 is the transformed AABB.
    // ========================================================

    if (json.find(L"\"type\":\"regions\"") == std::wstring::npos)
        return S_OK;

    double generationValue = -1.0;
    ExtractJsonNumber(json, L"generation", generationValue);

    const long long generation =
        static_cast<long long>(generationValue);

    /*
        Snapshot semantics: the browser message is authoritative.
        A region absent from this generation owns no native input.
    */
    HideAllInputPieces();

    int appliedCount = 0;

    for (int sirId = 0; sirId < 4; ++sirId)
    {
        const std::wstring idToken =
            L"\"id\":\"s" + std::to_wstring(sirId) + L"\"";

        const size_t idPos = json.find(idToken);
        ...
        if (idPos == std::wstring::npos)
        {
            std::cout
                ...
                << " | g=" << generation
                << " | S" << sirId
                << " | ABSENT -> input released\n";

            continue;
        }

        /*
            Restrict parsing to this region object. The next "id"
            marks the next region; otherwise parse to the end of
            the message. ExtractJsonNumber then sees this region's
            rect values rather than another region's values.
        */
        const size_t nextIdPos =
            json.find(L"\"id\":\"s", idPos + idToken.length());

        const std::wstring regionJson =
            json.substr(
                idPos,
                nextIdPos == std::wstring::npos
                    ? std::wstring::npos
                    : nextIdPos - idPos
            );
        ...
        if (!ParseInteractiveRegionSnapshot(regionJson, css))
        {
            std::cout
                << "P6F1"
                << " | g=" << generation
                << " | S" << sirId
                << " | INVALID geometry -> input released\n";

            continue;
        }

        const NativeRect native =
            CssToNativeRect(css);

        const bool applied =
            ApplyNativeInputPiece(
                sirId,      // stable native slot for S0..S3
                sirId,
                0,          // F1 has one rectangular piece per SIR
                generation,
                native,
                true
            );
        ...
        std::cout
            ...
            << " | g=" << generation
            << " | S" << sirId
            << " | css=[" << css.left << "," << css.top
            << " -> " << css.right << "," << css.bottom << "]"
            << " | native=[" << native.left << "," << native.top
...
Log(
    "P6F1 regions-snapshot handler installed"
);
                                    // ------------------------
                                    // Transparent WebView
                                    // ------------------------
...
)
{
    ...
    Log(
    "Hotoe Prototype 06F / F1"
);

Log(
    "Visible DOM Support -> Dynamic Native Input Topology"
);
